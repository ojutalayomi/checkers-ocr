...
def parse_cleaned_string(combined_string):
    """Extracts the exact delivery note identity, isolating it from the date."""
...
def run_mac_ocr(cropped_img):
    ...
    img_data = Cocoa.NSData.dataWithBytes_length_(img_byte_arr.getvalue(), len(img_byte_arr.getvalue()))
...
class BatchOCRScannerApp(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Container(id="main-container"):
            ...
            yield Input(placeholder="Enter absolute directory path (e.g., /Users/name/Documents/DeliveryNotes)",
                        id="folder-input")
            yield Button("Begin Batch Scan & Auto-Rename Pipeline", id="batch-scan-btn", variant="success")
    ...
    def on_mount(self) -> None:
        ...
        table.add_columns("Original File Name", "Extracted ID", "New File Name", "Status")
    ...
    async def on_button_pressed(self, event: Button.Pressed) -> None:
        if event.button.id == "batch-scan-btn":
            ...
            if not folder_path or not os.path.isdir(folder_path):
                logger.write_line("[ERROR] Invalid directory target. Please check the folder pathway.")
                return
            ...
            target_images = [f for f in all_files if f.lower().endswith(supported_extensions)]

            if not target_images:
                logger.write_line(f"[INFO] No compatible image assets discovered in folder target.")
                return

            logger.write_line(f"[START] Batch pipeline active. Processing {len(target_images)} files...")
            ...
            for idx, filename in enumerate(target_images, 1):
                ...
                logger.write_line(f"-> {extract_number_from_text(note_number, logger)}: {filename}")

                # 2. Handle Renaming Actions based on extraction success
                if "DN-" in note_number:
                    file_ext = os.path.splitext(filename)[1]  # Keeps original format extension (.jpg, .png, etc)
                    new_filename = f"{extract_number_from_text(note_number, logger)}{file_ext}"
                    new_full_path = os.path.join(folder_path, new_filename)

                    if filename == new_filename:
                        status_style = "[yellow]ALREADY RENAMED[/yellow]"
                        logger.write_line(f"Skipping: '{filename}' is already correctly named.")
                    elif os.path.exists(new_full_path):
                        status_style = "[orange3]NAME CONFLICT[/orange3]"
                        logger.write_line(f"[WARN] File '{new_filename}' already exists! Skipping to protect data.")
                    else:
                        try:
                            os.rename(full_path, new_full_path)
                            status_style = "[green]RENAMED[/green]"
                            logger.write_line(f"Successfully moved: '{filename}' ➡️ '{new_filename}'")
                        except Exception as e:
                            status_style = "[red]RENAME ERROR[/red]"
                            logger.write_line(f"[ERROR] OS File system locked: {str(e)}")
                else:
                    new_filename = "N/A"
                    status_style = "[red]OCR FAILED[/red]"
                    logger.write_line(f"[WARN] Could not safely identify index values for {filename}")

                # 3. Dynamically push a row straight to the live terminal grid
                table.add_row(filename, note_number, new_filename, status_style)
                self.refresh()

            logger.write_line("[FINISH] Batch system execution pipeline completed smoothly.")
